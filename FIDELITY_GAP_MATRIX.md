...
| Device-motion permission and shake activation | Use Aedicule's registered semantic shake event and retain a touch fallback | The host capability now exists; guest registration, event handling, and a visible no-sensor/denied fallback remain. |
| Browser touch controls | Keep desktop pointer controls live beside a bounded multi-contact guest table using Aedicule kinds 11--14 | The guest owns zones, strokes, and per-contact action ownership; the deployed host owns opaque IDs, ordered delivery, cancellation, AVP occlusion, and compatibility-pointer suppression. |
...
| Bullet origin, velocity inheritance, cadence, edge termination, and cap | **Implemented, deliberate lifetime deviation** | Player shots persist until a swept hit or until they clear the viewport's 25-pixel visual margin; they never wrap. Tests cover cadence/speed, zero-net-velocity overlap before inert cleanup, edge exit without re-entry, and full-pool failure. |
...
| Ship death, debris, safe respawn, bomb, and game over | **Implemented** | World effects keep advancing through the death sequence; source-time lifecycle boundaries, 96/48 safe radii, no-score bomb, delayed game over, and a consumed-input fresh-game gate are tested. |
| Start, Resume, pause, focus release, restart | **Implemented; awaiting gate visual approval** | Boot has no ship while rocks drift. Focus loss clears held edges and freezes behind Resume. Standalone Start/Resume actions back one retained native button without adding menu items; its semantic action or any key dismisses and is consumed. Raw canvas pointer edges cannot activate it. |
...
| Multi-contact touch controls | **Implemented; Peter-approved on iPhone** | WAST and the pinned actual-binary timeline prove simultaneous edge fire/steering and center thrust, per-contact end/cancel, shared owners, ID reuse, duplicate/unknown safety, `4pi` strokes, keyboard coexistence, and focus/reload cleanup. Peter confirmed the live Safari mapping on 2026-08-14. |
